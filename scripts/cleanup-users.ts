...
async function main() {
  if (process.env.NODE_ENV === "production") {
    throw new Error("cleanup-users cannot run in production");
  }
  ...
  const password = process.env.CLEANUP_ADMIN_PASSWORD;
  if (!password) {
    throw new Error("CLEANUP_ADMIN_PASSWORD environment variable is required");
  }
  ...
  const oldExecs = [
    "se1@sukisoftware.com",
    "se2@sukisoftware.com",
    "exec1@sukisoftware.com",
    "exec2@sukisoftware.com",
    "exec3@sukisoftware.com",
    "exec4@sukisoftware.com",
    "lead@sukisoftware.com",
  ];
  ...
  for (const email of oldExecs) {
    ...
    if (u) {
      await prisma.user.update({
        where: { email },
        data: { email: "OLD_" + email, isActive: false },
      });
    }
  }
