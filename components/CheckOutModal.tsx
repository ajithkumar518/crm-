...
import {
  checkOutInboundAction,
  checkOutOutboundAction,
...
const COMMON_OUTCOMES = [
  ...
  {
    value: "Proposal Needed",
    label: "📋 Proposal Needed",
    color: "text-[var(--primary)]   bg-red-50   border-red-200",
  },
  {
    value: "Negotiation Ongoing",
...
const PORTAL_DECISION_PURPOSES = new Set([
  "New Enquiry",
  "Sales Meeting",
  "Product Demo",
  "Sales Pitch",
...
export default function CheckOutModal({
  ...
  const fetchGPS = () => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          setLocError(
            "GPS capture failed. You may retry or proceed without location.",
          );
          setLocLoading(false);
        },
        ...
        <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="bg-[var(--primary)] px-6 py-6 text-center">
            ...
            <p className="text-white mt-1 font-medium">
              {checkedOutName} — visit logged successfully
            </p>
          </div>
  ...
  const showPortalDecision =
    PORTAL_DECISION_PURPOSES.has(visit.purpose) && !isLost;
  ...
  const inputCls =
    "w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200/80 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/20 focus:border-[var(--primary)] text-slate-700 font-medium transition-all";
          ...
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-red-50 to-slate-50 shrink-0">
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                {visit.visitType === "Inbound"
                  ? "Capture visit end, outcome, and decisions"
                  : "Record field meeting outcomes and status"}
              </p>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-700 transition-all"
            >
              <svg
                className="w-4 h-4"
                fill="none"
          ...
          >
            <div className="p-6 overflow-y-auto space-y-4 flex-1">
              ...
              <div className="p-4 border border-red-100 bg-red-50/40 rounded-2xl flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-bold text-[#C94F4F] uppercase tracking-wider">
                    Active Visit
                  </p>
                  ...
                  <p className="text-xs text-slate-500 font-semibold">
                    {visit.customerCode} · {visit.visitType} · {visit.purpose}
                  </p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs font-bold text-slate-600 mt-1">
                    {new Date(visit.checkInTime).toLocaleTimeString("en-IN", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </p>
                  ...
                  <div className="flex-1">
                    ...
                    {location && (
                      <p className="text-xs font-semibold text-emerald-600 flex items-center gap-1.5">
                        ...
                        Locked: {location.lat.toFixed(5)}°N,{" "}
              ...
              <div>
                <label className={labelCls}>
                  Meeting Summary / Discussion Details{" "}
                  <span className="text-red-400">*</span>
                </label>
                <textarea
                  rows={3}
                  required
                  placeholder="Detail what was discussed, decisions made, follow-up actions, key points raised..."
                  value={meetingNotes}
                ...
                <div className="bg-amber-50/60 p-4 border border-amber-100 rounded-2xl space-y-3 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="Agenda for next meeting..."
              ...
              {isWon && outcome === "Closed Won" && (
                <div className="bg-emerald-50/60 p-4 border border-emerald-100 rounded-2xl animate-in slide-in-from-top-2 duration-200">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={confirmClosedWon}
                      onChange={(e) => setConfirmClosedWon(e.target.checked)}
                      className="w-4 h-4 rounded text-emerald-600"
                    />
                    <span className="text-xs font-semibold text-slate-700">
                      Confirm: Promote customer to Active status in CRM
                    </span>
                  </label>
                </div>
              )}
              ...
              {isLost && (
                <div className="bg-red-50/60 p-4 border border-red-100 rounded-2xl animate-in slide-in-from-top-2 duration-200">
                  <p className="text-[10px] font-black text-red-700 uppercase tracking-wider mb-2">
                    ❌ Deal Closed Lost
                  </p>
                  <div>
                    <label className={labelCls}>
                    ...
                    <input
                      type="text"
                      required={isLost}
                      placeholder="Why did the customer not proceed? (budget, competitor, timing...)"
                      value={lossReason}
              ...
              {showPortalDecision && (
                <div>
                  <label className={labelCls}>
                    Customer Portal Access Decision{" "}
                    <span className="text-red-400">*</span>
                  </label>
                  <div className="flex gap-2 p-1.5 bg-slate-100 rounded-xl">
                    {[
                      ["APPROVED", "Approve ✓", "text-emerald-600"],
                      ["REJECTED", "Reject ✗", "text-red-600"],
                      ["PENDING", "Pending", "text-slate-800"],
                    ].map(([val, lbl, color]) => (
                      <label
                        key={val}
                        className={`flex-1 text-center py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${customerDecision === val ? `bg-white ${color} shadow-sm` : "text-slate-500"}`}
                      >
                        <input
                          type="radio"
                          value={val}
                          checked={customerDecision === val}
                          onChange={() => setCustomerDecision(val)}
                          className="hidden"
                        />
                        {lbl}
                      </label>
              ...
              {customerDecision === "REJECTED" && showPortalDecision && (
                <div className="animate-in slide-in-from-top-2 duration-200">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="Reason for rejecting portal access..."
            ...
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 shrink-0">
              ...
              <button
                ...
                className="px-7 py-2.5 rounded-xl text-xs font-bold text-white bg-[var(--primary)] hover:bg-[var(--primary-hover)] transition-colors shadow-sm disabled:opacity-75 flex items-center gap-2 cursor-pointer"
              >
                {formLoading ? (
                  <>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in-95 duration-150">
            ...
            <h3 className="text-sm font-bold text-slate-800 text-center">
              Confirm Customer Status Change
            </h3>
            <p className="text-xs text-slate-500 text-center mt-2 leading-relaxed">
              Are you sure you want to{" "}
              <strong>
                {customerDecision === "APPROVED" ? "APPROVE" : "REJECT"}
              </strong>{" "}
              portal access for {visit.customerName}?
              {customerDecision === "APPROVED"
                ? " This will send an account activation link to their email."
                : " This will log the rejection and block portal login."}
            </p>
