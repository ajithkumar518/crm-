...
export const SalesIcons = {
  ...
  conversion:
    "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 002 2h2a2 2 0 002-2",
  chart:
    "M7 12l3-3 3 3 4-4M8 21h12a2 2 0 002-2V7a2 2 0 00-2-2H8a2 2 0 00-2 2v12a2 2 0 002 2z",
};
...
export function AnalyticsCard({
  ...
  return (
    <div
      className={cn(
        "crm-card p-5 flex flex-col min-h-[360px] h-full",
        className,
      )}
    >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
        <div>
          <p className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">
            Active Leads
          </p>
          <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100">
            <CountUp end={kpis.totalLeads || 0} />
          </h3>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
        <div>
          ...
          <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100">
            {(() => {
              const p = parseCountValue(
                formatCurrency(kpis.pipelineRevenue || 0),
              );
              return (
                <CountUp end={p.end} prefix={p.prefix} decimals={p.decimals} />
              );
            })()}
          </h3>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 p-6 rounded-3xl shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
        <div>
          ...
          <h3 className="text-3xl font-black text-slate-800 dark:text-slate-100">
            <CountUp end={kpis.conversionRate || 0} suffix="%" decimals={1} />
          </h3>
          <p className="text-[10px] font-bold mt-1.5 text-slate-400 dark:text-slate-500">
            {kpis.wonDeals} Won / {(kpis.openDeals || 0) + (kpis.wonDeals || 0)}{" "}
            Total
          </p>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="w-24 shrink-0 text-right pr-4">
                <span
                  className={`text-xs font-bold transition-colors ${isHovered ? "text-slate-800" : "text-slate-500"}`}
                >
                  {item.stage.split(" ")[0]}
                </span>
              </div>
              ...
              <div
                className={`absolute left-28 bg-slate-800 text-white text-[10px] font-bold py-1 px-2.5 rounded-lg whitespace-nowrap transition-all duration-200 z-10 flex items-center gap-1.5 pointer-events-none ${isHovered ? "opacity-100 translate-x-2" : "opacity-0 translate-x-0"}`}
              >
...
export function RevenueTrendChart({ revenueTrend }: { revenueTrend: any[] }) {
  ...
  const maxTrendRevenue = Math.max(
    ...revenueTrend.map((t: any) => t.revenue),
    1000,
  );
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  stroke={isHovered ? "#8B5CF6" : "#FFFFFF"}
                ...
                <text
                  ...
                  fill="#8B5CF6"
                  textAnchor="middle"
                  className={`transition-opacity duration-150 ${isHovered ? "opacity-100 -translate-y-1" : "opacity-0"}`}
                >
            ...
            <tbody className="divide-y divide-slate-50 font-medium text-slate-700">
              {leadSources.map((ls: any, idx: number) => (
                ...
                >
                  ...
                  <td className="p-3 text-right font-black text-[#8B5CF6]">
                    {formatCurrency(ls.revenue || 0)}
                  </td>
                </tr>
              ))}
            </tbody>
...
export function AgentLeaderboard({
  ...
  return (
    <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-6 rounded-[24px] h-[340px] flex flex-col shadow-sm transition-colors duration-300">
      <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-6 shrink-0">
        Executive Sales Leaderboard
      </h3>
      <div className="space-y-4 flex-1 overflow-y-auto pr-1 min-h-0">
        {!agentPerformance || agentPerformance.length === 0 ? (
          <p className="text-xs text-slate-500 italic text-center py-6">
            Leaderboard metrics unavailable.
          </p>
        ) : (
          agentPerformance.map((agent: any, idx: number) => (
            <div
              key={idx}
              className="flex items-center justify-between p-4 border border-slate-100 rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow"
            >
              <div className="flex items-center gap-3">
                <span
                  className={`w-8 h-8 rounded-xl text-xs font-black flex items-center justify-center shrink-0 ${idx === 0 ? "bg-amber-100 text-amber-600" : idx === 1 ? "bg-slate-100 text-slate-600" : idx === 2 ? "bg-orange-100 text-orange-600" : "bg-blue-50 text-blue-500"}`}
                >
                  #{idx + 1}
                </span>
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold mt-0.5">
                    Deals won: {agent.wonCount}/{agent.dealsCount}{" "}
...
const getStatusBadge = (status: string) => {
  switch (status) {
    case "Qualified":
      return {
        label: "Qualified",
        pill: "bg-blue-50 text-blue-600 dark:bg-blue-950/20 dark:text-blue-400 border border-blue-100/50 dark:border-blue-900/30",
        dot: "bg-blue-500",
      };
    case "Contacted":
      return {
        label: "Contacted",
        pill: "bg-cyan-50 text-cyan-600 dark:bg-cyan-950/20 dark:text-cyan-400 border border-cyan-100/50 dark:border-cyan-900/30",
        dot: "bg-cyan-500",
      };
    case "Follow-up":
    case "FollowUp":
      return {
        label: "Follow-up",
        pill: "bg-amber-50 text-amber-600 dark:bg-amber-950/20 dark:text-amber-400 border border-amber-100/50 dark:border-amber-900/30",
        dot: "bg-amber-500",
      };
    case "Won":
      return {
        label: "Won",
        pill: "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-900/30",
        dot: "bg-emerald-500",
      };
    case "Lost":
      return {
        label: "Lost",
        pill: "bg-rose-50 text-rose-600 dark:bg-rose-950/20 dark:text-rose-400 border border-rose-100/50 dark:border-rose-900/30",
        dot: "bg-rose-500",
      };
    default:
      return {
        label: status,
        pill: "bg-slate-50 text-slate-600 dark:bg-slate-900 dark:text-slate-400 border border-slate-100 dark:border-slate-800",
        dot: "bg-slate-500",
      };
  }
};
...
export function WorkspaceOverviewLineChart({
  ...
  const getScaledData = (totalVal: number, pattern: number[]) => {
    ...
    return pattern.map(
      (v) => Math.round((v / patternSum) * totalVal * 10) / 10,
    );
  };
...
export function SalesPipelineWidget({
    ...
    datasets: [
      {
        ...
        backgroundColor: ["rgba(232, 98, 10, 0.85)", "#3b82f6", "#10b981"],
        borderWidth: 0,
        cutout: "75%",
      },
    ],
  ...
  if (funnel && funnel.length > 0) {
    stages[0].count =
      (funnel.find((f: any) => f.stage === "New Lead")?.count || 0) +
      (funnel.find((f: any) => f.stage === "Contacted")?.count || 0);
    stages[1].count =
      funnel.find((f: any) => f.stage === "Qualified")?.count || 0;
    stages[2].count =
      funnel.find((f: any) => f.stage === "Meeting Scheduled")?.count || 0;
    stages[3].count =
      funnel.find((f: any) => f.stage === "Active Deal")?.count || 0;
    stages[4].count =
      funnel.find((f: any) => f.stage === "Closed Won")?.count || 0;
  }
        ...
        <div className="relative w-28 h-28 shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
            <span className="text-2xl font-black text-slate-800 dark:text-slate-200 tracking-tight">
              {total === 1 && activeLeads + visits + subscriptions === 0
                ? 0
                : total}
            </span>
        ...
        <div className="flex flex-col gap-2.5 flex-1 text-xs">
          <div className="flex items-center justify-between border-b border-slate-50 dark:border-slate-800 pb-1.5">
            ...
            <span className="font-black text-slate-700 dark:text-slate-300">
              {activeLeads} ({Math.round(leadsPct * 100)}%)
            </span>
          </div>
          <div className="flex items-center justify-between border-b border-slate-50 dark:border-slate-800 pb-1.5">
            <div className="flex items-center gap-2">
            ...
            <span className="font-black text-slate-700 dark:text-slate-300">
              {visits} ({Math.round(visitsPct * 100)}%)
            </span>
          </div>
          <div className="flex items-center justify-between">
            ...
            <span className="font-black text-slate-700 dark:text-slate-300">
              {subscriptions} ({Math.round(subsPct * 100)}%)
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 flex-1 flex flex-col justify-center">
        {stages.map((stage, idx) => (
          <div key={idx} className="flex items-center justify-between text-xs">
            <span className="w-20 font-bold text-slate-500 truncate">
              {stage.label}
            </span>
            <div className="flex-1 mx-3 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${stage.pct}%`,
                  backgroundColor:
                    stage.label === "Won" ? "#10b981" : accentColor,
                }}
              />
            </div>
                  ...
                  >
                    <td className="p-3">
                      <div className="flex items-center gap-2.5">
                        <div
                          className={cn(
                            "w-7 h-7 rounded-full text-[10px] font-black flex items-center justify-center shrink-0 shadow-sm",
...
export function ActionRequiredWidget({
  ...
  const mapped = (followUps || []).map((f, idx) => {
    ...
    if (remarksLower.includes("email") || remarksLower.includes("mail")) {
      ...
      remarksLower.includes("demo") ||
      remarksLower.includes("meeting") ||
      remarksLower.includes("schedule")
    ) {
      type = "Meeting";
    } else if (
      remarksLower.includes("proposal") ||
      remarksLower.includes("quote") ||
      remarksLower.includes("offer")
    ) {
      type = "Proposal";
    } else if (
      remarksLower.includes("task") ||
      remarksLower.includes("review") ||
      remarksLower.includes("contract")
    ) {
      type = "Task";
    } else {
      const typeList: Array<
        "Call" | "Email" | "Meeting" | "Proposal" | "Task"
      > = ["Call", "Email", "Meeting", "Proposal", "Task"];
      type = typeList[idx % 5];
    }
    ...
    if (isCompleted) {
      ...
      if (diffHrs < 2) {
        ...
        statusText = date.toLocaleTimeString("en-IN", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: true,
        });
  ...
  const mockItems = [
    ...
    {
      type: "Email" as const,
      title: "Follow-up action item 11",
      client: "Nexus Corp",
      statusText: "2h left",
      statusColorClass: "text-amber-500 dark:text-amber-400",
    },
    {
      ...
      title: "Demo Scheduled — Priya Sharma",
      client: "TechBridge Pvt Ltd",
      statusText: "3:00 PM",
      statusColorClass: "text-slate-400 dark:text-slate-500",
    },
    {
      type: "Proposal" as const,
      title: "Proposal sent — Ramesh K",
  ...
  return (
    <AnalyticsCard
      ...
      footer={
        <a
          href="/follow-up"
          className="btn-primary w-full justify-center text-xs font-bold h-9"
        >
    ...
    >
      <div className="flex-1 flex flex-col justify-between gap-3 my-2">
        {displayItems.map((item) => (
          <div
            key={item.id}
            className="flex items-center justify-between py-1.5 border-b border-slate-50 dark:border-slate-800/40 last:border-0"
          >
...
export function PipelinePieChart({ funnel }: { funnel: any[] }) {
  ...
  if (!funnel || funnel.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-[24px] p-6 h-full flex flex-col items-center justify-center">
        <h3 className="text-[15px] font-bold text-slate-900 dark:text-slate-100 mb-4">
  ...
  return (
    <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-[24px] p-6 h-full flex flex-col justify-between shadow-sm transition-colors duration-300">
      <h3 className="text-[15px] font-bold text-slate-900 dark:text-slate-100 text-center mb-6">
        Pipeline Performance
      </h3>
...
export function CustomerScoreTrendChart({ scoreTrend }: { scoreTrend: any[] }) {
  ...
  if (!scoreTrend || scoreTrend.length === 0) {
    return (
      ...
      >
        <AnalyticsEmptyState>
          No customer score data yet. Conduct demos and record ratings to see
          trends.
        </AnalyticsEmptyState>
      </AnalyticsCard>
    );
...
export function OverallCRMHealthScore({
  kpis,
  crossModule,
}: {
  kpis: any;
  ...
  const activityScore = Math.min(
    ((crossModule?.samples?.total || 0) + (crossModule?.rfq?.total || 0)) * 2,
    50,
  );
...
export function RecentActivityFeed({
  ...
  if (inactiveDeals > 0) {
    alerts.push({
      type: "risk",
      colorClass:
        "bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-100/30",
      icon: <Clock size={14} />,
  ...
  if (overdueFollowUps > 0) {
    alerts.push({
      type: "warning",
      colorClass:
        "bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 border border-amber-100/30",
  ...
  if (pendingRFQs > 0) {
    alerts.push({
      type: "rfq",
      colorClass:
        "bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 border border-indigo-100/30",
    ...
    <div className="bg-[var(--surface)] border border-[var(--border-subtle)] p-6 rounded-[24px] h-[340px] flex flex-col justify-between hover:shadow-[0_8px_30px_rgba(0,0,0,0.03)] transition-all duration-300">
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-4 min-h-0">
        {alerts.slice(0, 4).map((alert, idx) => (
          <div
            key={idx}
            className="flex gap-3 relative py-1 border-b border-[var(--border-subtle)] last:border-0 pb-3 last:pb-0"
          >
            <div
              className={cn(
...
export function LeadSourceDoughnut({ leadSources }: { leadSources: any[] }) {
  ...
  const validSources = (leadSources || [])
    .filter(
      (ls: any) => ls.source && ls.source !== "Unknown" && ls.source !== "",
    )
    .slice(0, 4);
      ...
      tooltip: {
        callbacks: {
          label: (ctx: any) => {
            const val = ctx.raw;
            const pct =
              totalLeads > 0 ? Math.round((val / totalLeads) * 100) : 0;
            return ` ${ctx.label}: ${val} (${pct}%)`;
          },
        },
      },
      ...
      {totalLeads === 0 ? (
        ...
        <>
          <div className="relative w-full h-[140px] flex items-center justify-center shrink-0 mt-2">
            <Doughnut data={chartData} options={options} />
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mt-1">
              <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider mb-0.5">
                Leads
              </span>
              <span className="text-2xl font-black text-[var(--text-primary)] tracking-tight leading-none">
                {totalLeads}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4 text-[10px] font-bold text-[var(--text-muted)]">
            {validSources.map((ls: any, idx: number) => (
              <div
                key={idx}
                className="flex items-center justify-between border-b border-[var(--border-subtle)] pb-1.5 last:border-0"
              >
                ...
                <span className="text-[var(--text-primary)] pl-1 shrink-0">
                  {ls.count} (
                  {totalLeads > 0
                    ? Math.round((ls.count / totalLeads) * 100)
                    : 0}
                  %)
                </span>
              </div>
          ...
          <div className="relative w-20 h-20 shrink-0">
            ...
            >
              ...
              <circle
                ...
                strokeDashoffset={
                  2 * Math.PI * 26 - (conversionRate / 100) * (2 * Math.PI * 26)
                }
                strokeLinecap="round"
                className="transition-all duration-1000 ease-out"
              />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none mt-0.5">
              <span className="text-base font-extrabold text-[var(--text-primary)] tracking-tighter leading-none">
                {conversionRate}%
              </span>
              <span className="text-[7px] font-bold text-[var(--text-muted)] uppercase tracking-wider mt-0.5">
                Won
              </span>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <h4 className="text-xs font-extrabold text-[var(--text-primary)] leading-snug">
              Overall Conversion Rate
            </h4>
            <p className="text-[11px] text-[var(--text-muted)] font-medium mt-1 leading-snug">
              Ratio of closed deals won successfully. Higher conversions
              indicate efficient pipeline management.
            </p>
          </div>
        </div>
...
export function ForecastVsTargetWidget({
  kpis,
  agentPerformance,
}: {
  kpis: any;
      ...
      <div className="flex-1 flex flex-col justify-around py-2">
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex justify-between text-[10px] font-bold text-[var(--text-muted)] mt-1">
              <span>
                Potential Quota Met: {Math.min(achievedPct + forecastPct, 100)}%
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="p-3 bg-[var(--surface-hover)] border border-[var(--border-subtle)] rounded-2xl text-[10px] text-[var(--text-muted)] leading-relaxed font-semibold">
          💡 <strong>Forecast Alert</strong>: Closing all open deals will hit{" "}
          <strong>{Math.round(((achieved + forecast) / target) * 100)}%</strong>{" "}
          of your target quota this month.
        </div>
      </div>
    </div>
  );
...
export function FollowUpTrendWidget({
  followUpMetrics,
}: {
  followUpMetrics: any;
}) {
        ...
        <div className="mt-2">
          ...
          <div className="w-full h-2.5 bg-[var(--surface-hover)] border border-[var(--border-subtle)] rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${pending > 0 ? Math.round((overdue / pending) * 100) : 0}%`,
              }}
            />
          </div>
        </div>
