...
import {
  Upload, Download, CheckCircle2, AlertTriangle, AlertCircle,
  Loader2, ChevronLeft,
} from "lucide-react";
...
interface ImportResponse {
  ...
  errors: number;
  details: any[];
  createdRows: { row: number; productCode: string; name: string }[];
}
...
export default function ProductImportModal({ open, onClose, onImportDone }: Props) {
  ...
  const runDryRun = useCallback(async (f: File) => {
    ...
    try {
      ...
      fd.append("file", f);
      const res = await fetch("/api/catalogue/products/import?dryRun=true", { method: "POST", body: fd });
  ...
  const handleImport = async () => {
    ...
    try {
      const fd = new FormData();
      fd.append("file", file);
      const res = await fetch("/api/catalogue/products/import", { method: "POST", body: fd });
      const data = await res.json();
      if (!data.success) {
  ...
  const downloadTemplate = () => {
    window.location.href = "/api/catalogue/products/import/template";
  };
  ...
  return (
    <Modal
      ...
      subtitle="Upload an Excel file to bulk import product master data"
      size="xl"
    >
                ...
                <tbody>
                  {preview.map((r) => (
                    <tr key={r.row} className={r.status !== "Valid" ? "bg-red-50 dark:bg-red-950" : ""}>
                      ...
                      <td className="px-3 py-1.5 text-muted-foreground">{r.productCode}</td>
          ...
          <div className="space-y-4">
            ...
            {result.created > 0 && (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-green-50 border border-green-200 dark:bg-green-950 dark:border-green-800">
                ...
                <p className="text-sm text-green-700 dark:text-green-300 font-medium">{result.created} products imported successfully!</p>
              </div>
            )}
          ...
          <div className="flex items-center gap-2">
            ...
            {step === 1 && (
              <button
                ...
                {importing ? <><Loader2 size={15} className="animate-spin" /> Importing…</> : `Import ${validCount} Product${validCount !== 1 ? "s" : ""}`}
              </button>
            )}
          </div>
        </div>
