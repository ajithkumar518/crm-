...
export async function GET(
  ...
  const guard = enforceModuleGuard(user, MODULE_KEYS.NEGOTIATION, "GET /api/negotiations/[id]/notes");
...
export async function POST(
  ...
  const user = await verifyAuth();
  if (!user) return NextResponse.json({ success: false, message: "Unauthorized" }, { status: 401 });
  const guard = enforceModuleGuard(user, MODULE_KEYS.NEGOTIATION, "POST /api/negotiations/[id]/notes");
  if (guard) return guard;
