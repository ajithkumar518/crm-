...
export default function CustomerImportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/customer-master/import?dryRun=${dryRun}`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Customer Master Import</h1>
      <p className="text-sm text-gray-600 mb-6">
        Upload an Excel file with columns: Customer Name, GST Number, Contact Person, Mobile Number, Email ID, Address, State, Payment Terms, Credit Days, Marketing Executive, Customer Category.
      </p>
                    ...
                    <tbody>
                      {result.details.map((r: any, i: number) => (
                        <tr key={i}>
                          ...
                          <td className="border p-2">{r.customerCode || r.productCode} - {r.name}</td>
