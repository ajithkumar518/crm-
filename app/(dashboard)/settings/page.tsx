...
import {
  ...
  updateSystemConfigsAction,
} from "@/app/actions/systemConfigs";
import {
...
import {
  updateCompanyVariantAction,
  updateCompanyModulesAction,
} from "@/app/actions/auth";
import {
  MODULE_KEYS,
  getModulesForVariant,
  type ModuleKey,
} from "@/lib/config/moduleVariantMap";
...
export default function SettingsPage() {
  ...
  const [companyVariant, setCompanyVariant] = useState<number>(
    user?.company?.variant || user?.variant || 1,
  );
...
<script>
  ...
  const handleSaveCurrency = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(
          "Currency preference saved. All amounts will now display in " +
            preferredCurrency +
            ".",
        );
      } else {
  ...
  const handleRegenerateApiKey = () => {
    if (
      confirm(
        "Are you sure you want to regenerate the API key? External forms using the old key will stop working.",
      )
    ) {
      ...
      const key =
        "suki_" +
        Array.from(array, (b) => b.toString(36).padStart(2, "0"))
          .join("")
          .slice(0, 24);
      setLeadsApiKey(key);
      toast.info(
        "New key generated. Please save preferences to apply changes.",
      );
    }
  };
  ...
  const canEditVariant =
    (user?.role === "SuperAdmin" || user?.role === "Admin") && !isPlanLocked;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {canEditVariant ? (
                  ...
                  <div className="py-2 px-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-600">
                    ...
                    <span className="block text-xs text-slate-400 mt-0.5">
                      Contact Suki Software to upgrade.
                    </span>
                  </div>
                )}
                {canEditVariant && (
            ...
            <div className="space-y-1">
              ...
              {canEditVariant ? (
                ...
                <div className="mt-4 py-3 px-4 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-600 text-center">
                  ...
                  <span className="font-bold text-slate-800">
                    Contact support to enable new modules.
                  </span>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  <div
                    className={`h-1.5 flex-1 rounded-full transition-colors ${pwScore >= 1 ? (pwScore >= 3 ? "bg-emerald-500" : pwScore >= 2 ? "bg-amber-400" : "bg-red-400") : "bg-slate-200"}`}
                  />
                  <div
                    className={`h-1.5 flex-1 rounded-full transition-colors ${pwScore >= 2 ? (pwScore >= 3 ? "bg-emerald-500" : "bg-amber-400") : "bg-slate-200"}`}
                  />
                  <div
                    className={`h-1.5 flex-1 rounded-full transition-colors ${pwScore >= 3 ? "bg-emerald-500" : "bg-slate-200"}`}
                  />
          ...
          <Card title="Notifications" icon={icons.bell}>
            ...
            <Toggle
              ...
              onChange={(c) =>
                setPrefs((p) => ({ ...p, emailVisitorCheckIn: c }))
              }
            />
            <Toggle
            ...
            <Toggle
              ...
              onChange={(c) =>
                setPrefs((p) => ({ ...p, notifyOnDealStatus: c }))
              }
            />
            <Toggle
          ...
          <Card title="Compliance & Privacy" icon={icons.shield}>
            <Toggle
              ...
              description="Marketing executives must share location on field visits."
              defaultChecked={true}
            />
            <Toggle
              label="Log All API Actions"
              description="Record every data-modification action in the audit log."
              defaultChecked={true}
            />
            <Toggle
              label="Restrict Admin Features"
              description="Limit User Master & Audit Log to Admin role only."
              defaultChecked={true}
            />
            <Toggle
              label="Two-Factor Authentication"
              description="Require 2FA for all Admin role accounts."
              defaultChecked={false}
            />
            <div className="mt-4 p-4 bg-amber-50 border border-amber-100 rounded-xl">
              ...
              <p className="text-[11px] text-amber-700 mt-1">
                Audit logs are retained for 90 days. Contact your administrator
                to configure custom retention policies.
              </p>
            </div>
          </Card>
          ...
          <Card
            title="Lead Ingestion API & Auto-Assignment"
            icon={icons.shield}
          >
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                >
                  <option value="ROUND_ROBIN">
                    Round-Robin Assignment (Balanced Load)
                  </option>
                  <option value="DEFAULT_POOL">
                    Triage Pool (Assign to Default User)
                  </option>
                </select>
              </div>
              ...
              {leadsAssignmentMode === "DEFAULT_POOL" && (
                <div>
                  ...
                  >
                    <option value="">
                      -- Select default triage assignee --
                    </option>
                    {users.map((u) => (
              ...
              <div className="mt-4 pt-4 border-t border-slate-100">
                ...
                <p className="text-[11px] text-slate-500 mb-2">
                  Copy and paste this HTML form and script block into your
                  company's landing page or external website.
                </p>
                <textarea
                  readOnly
                  value={codeSnippet}
