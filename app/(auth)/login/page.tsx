...
function getStrength(p: string): {
  ...
  if (score === 2)
    return { level: score, label: "Fair", color: "var(--warning)" };
  if (score === 3)
    return { level: score, label: "Good", color: "var(--success)" };
...
function LoginContent() {
  ...
  const [queryFlags, setQueryFlags] = useState({
    resetSuccess: false,
    activated: false,
    timedOut: false,
  });
  useEffect(() => {
    ...
    setQueryFlags({
      resetSuccess: sp.get("reset") === "success",
      activated: sp.get("activated") === "true",
      timedOut: sp.get("reason") === "timeout",
    });
  }, []);
  const { resetSuccess, activated, timedOut } = queryFlags;
  ...
  const ErrorBox = () =>
    error ? (
      <div className="mb-5 p-3 rounded-[8px] bg-[var(--error-bg)] border border-[var(--error-border)] text-[13px] text-[var(--error)] font-medium text-center">
        {error}
      </div>
    ) : null;
  ...
  const InfoBox = () =>
    info ? (
      <div className="mb-5 p-3 rounded-[8px] bg-[var(--success-bg)] border border-[var(--success-border)] text-[13px] text-[var(--success)] font-medium text-center">
        {info}
      </div>
    ) : null;
              ...
              <>
                ...
                >
                  <div>
                    <label
                      htmlFor="email"
                      className="block text-[11px] font-medium text-[var(--text-primary)] mb-2 tracking-[0.08em] uppercase"
                    >
                      Email Address
                    </label>
                    <input
              ...
              <>
                ...
                >
                  ...
                  <div>
                    ...
                    {confirmPassword && (
                      <p
                        className={`mt-1.5 text-[11px] font-medium ${passwordsMatch ? "text-[var(--success)]" : "text-[var(--error)]"}`}
                      >
                        {passwordsMatch
                          ? "✓ Passwords match"
                  ...
                  <label className="flex items-center gap-2.5 cursor-pointer select-none">
                    <div className="relative">
                      ...
                      />
                      <div
                        className={`w-4 h-4 rounded border-2 transition-all flex items-center justify-center ${rememberMe ? "border-[var(--brand-primary)]" : "border-[var(--border)] bg-[var(--surface-1)]"}`}
                        style={{
                      ...
                      >
                        {rememberMe && (
                          <svg
                            className="w-3 h-3 text-[var(--text-on-brand)]"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth={3}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <label
                        ...
                        className="text-[11px] font-medium text-[var(--text-primary)] tracking-[0.08em] uppercase"
                      >
                        Password
                      </label>
                      <Link
