  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        leads_default_assignee_id:
          configMap.get("leads_default_assignee_id") || "",
      },
    };
  } catch (error: any) {
...
export async function updateSystemConfigsAction(data: {
  ...
  try {
    ...
    const { leads_api_key, leads_assignment_mode, leads_default_assignee_id } =
      data;
    ...
    if (leads_assignment_mode !== undefined) {
      updates.push(
        prisma.systemConfig.upsert({
          ...
          create: {
            key: "leads_assignment_mode",
            value: leads_assignment_mode,
          },
        }),
      );
    }
    if (leads_default_assignee_id !== undefined) {
      updates.push(
        prisma.systemConfig.upsert({
          ...
          create: {
            key: "leads_default_assignee_id",
            value: leads_default_assignee_id,
          },
        }),
      );
    }
    ...
    await logAudit(
      ...
      "system-setting",
      "update",
      "System assignment & API configurations updated",
    );
